    ...
    <div className="min-h-screen bg-[#F4F7FB] flex flex-col md:flex-row font-sans text-slate-900">
      ...
      <div className="md:hidden bg-[#050A15] border-b border-white/10 px-4 py-4 flex items-center justify-between sticky top-0 z-50">
        <div className="font-black text-xl tracking-tight text-white flex items-center gap-2">
          <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-[var(--color-brand-blue)] to-[var(--color-brand-orange)] flex items-center justify-center">
              ...
              {navItems.map(item => {
                ...
                return (
                  <Link 
                    ...
                    className={`flex items-center gap-4 px-4 py-3.5 rounded-xl font-bold transition-all ${isActive ? 'bg-[var(--color-brand-blue)]/20 text-blue-300 border border-blue-500/30 shadow-inner' : 'text-slate-400 hover:bg-white/5 hover:text-white border border-transparent'}`}
                  >
                    <item.icon className={`w-5 h-5 ${isActive ? 'text-[var(--color-brand-orange)]' : 'text-slate-500'}`} />
                    {item.name}
                  </Link>
                );
              })}
      ...
      >
        {/* Subtle orange mesh in the sidebar background */}
        <div className="absolute top-0 left-0 w-full h-64 bg-[var(--color-brand-blue)]/5 blur-3xl rounded-full pointer-events-none"></div>
          ...
          {navItems.map(item => {
            ...
            return (
              <Link 
                ...
                className={`flex items-center gap-4 px-3.5 py-3 rounded-2xl font-bold transition-all duration-300 ${isActive ? 'bg-gradient-to-r from-[var(--color-brand-blue)]/20 to-[var(--color-brand-blue)]/5 text-blue-300 border border-blue-500/20 shadow-inner' : 'text-slate-400 hover:bg-white/5 hover:text-white border border-transparent'} ${!sidebarOpen && 'justify-center'}`}
              >
                <item.icon className={`w-5 h-5 shrink-0 transition-colors ${isActive ? 'text-[var(--color-brand-orange)]' : 'text-slate-500 group-hover:text-white'}`} />
        ...
        <div className="p-4 border-t border-white/5 shrink-0 relative z-10 bg-black/20">
          ...
          <div className={`mt-4 flex items-center gap-3 ${!sidebarOpen && 'justify-center'}`}>
            <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-blue-900 to-slate-800 border border-white/10 flex items-center justify-center shrink-0">
            ...
            {sidebarOpen && (
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-blue-400 font-medium truncate">admin@alphaedu.ng</p>
              </div>
            )}
          </div>
        </div>
      ...
      <main className="flex-1 flex flex-col min-w-0 min-h-screen relative">
        {/* Decorative Top Gradient for Main Content area to ensure warmth */}
